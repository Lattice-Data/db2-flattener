...
class DB2Gatherer:
    ...
    def resolve_references_for_samples(self, all_samples):
        ...
        self.resolved_objects["ControlledTerm"] = self.fetch_controlled_terms(controlled_term_refs)
        ...
        for sample in samples:
            ...
            for field_name, ref_types in config.get("references", {}).items():
                ...
                if "controlled_terms" in ref_type_list:
                    continue
                for ref in extract_references_from_field(
                    sample.get(field_name), field_name, self.configs
                ):
                    resolved_obj = None
    ...
    def gather_complete_library_data(self, matrix_file_set_uuid):
        ...
        print(
            f"Found {len(sequence_file_uuids)} sequence file UUIDs referenced by raw matrix files"
        )
        ...
        print(
            f"Successfully fetched {len(all_libraries)} libraries total "
                        ...
                        if file_set:
                            ...
                            if library_ref:
                                ...
                                if lib_uuid in libraries_data:
                                    bucket = matched_data.setdefault(
                                        lib_uuid, {"sequence_files": {}, "sequence_file_sets": {}}
                                    )
