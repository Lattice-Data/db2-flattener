...
def strip_author_metadata_column_prefix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rename = {
        c: re.sub(r"^.*?_author_metadata_", "", c) for c in df.columns if "_author_metadata_" in c
    }
...
def join_sequence_column(df: pd.DataFrame, col: str, sep: str = "|") -> pd.DataFrame:
    ...
    if col not in df.columns:
        return df
    df = df.copy()
    df[col] = df[col].apply(lambda x: sep.join(map(str, x)) if isinstance(x, (list, tuple)) else x)
    return df
...
def cell_has_term_name_structure(val) -> bool:
    ...
    if isinstance(val, list):
        return any(isinstance(item, dict) and "term_name" in item for item in val)
    return False
