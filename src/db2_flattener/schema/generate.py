...
from db2_flattener.schema.extract import (
    LatticeProfileClient,
    ProfileSummary,
    summarize_profile,
)

YAML_PATH = Path(__file__).parent / "data" / "constants.yaml"
...
def get_concrete_classes(input_name: str, hierarchy_dict: Hierarchy) -> list[str] | str:
    ...
    if schema_id.api_name in hierarchy_dict and hierarchy_dict[schema_id.api_name]:
...
def create_object_config(profiles: JSONProfile) -> ObjectConfig:
    ...
    for schema in profiles["profiles"]:
        ...
        fields = [
            field["name"] for field in schema["properties"] if field["name"] not in EXCLUDED_FIELDS
        ]
...
def load_yaml_config(yaml_path: PathLike | str) -> dict[str, ConstantYAML]:
    ...
    return {endpoint: ConstantYAML(**values) for endpoint, values in full_yaml_config.items()}
...
def load_and_return_constant_dicts(mode: str) -> tuple[FieldTypes, ObjectConfig]:
    ...
    if endpoint in loaded_config:
        ...
        field_hash_mismatch = (
            loaded_config[endpoint].field_types_sha256 != new_config[endpoint].field_types_sha256
        )
        object_hash_mismatch = (
            loaded_config[endpoint].object_config_sha256
            != new_config[endpoint].object_config_sha256
        )
    ...
    if update_config or (endpoint not in loaded_config and endpoint in CONFIGS_TO_SAVE):
