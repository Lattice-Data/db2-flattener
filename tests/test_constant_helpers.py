...
@pytest.mark.parametrize("input,expected", [(name.slug, name.url_prefix) for name in NAMES])
def test_make_plural(input, expected):
    assert make_plural(input) == expected


@pytest.mark.parametrize("input,expected", [(name.url_prefix, name.slug) for name in NAMES])
def test_make_singular(input, expected):
    assert make_singular(input) == expected


@pytest.mark.parametrize("input,expected", [(name.slug, name.api_name) for name in NAMES])
def test_snake_to_camel(input, expected):
    assert snake_to_camel(input) == expected


@pytest.mark.parametrize("input,expected", [(name.api_name, name.slug) for name in NAMES])
def test_camel_to_snake(input, expected):
    assert camel_to_snake(input) == expected


@pytest.mark.parametrize(
    "api_name,slug,url_prefix", [(name.api_name, name.slug, name.url_prefix) for name in NAMES]
)
