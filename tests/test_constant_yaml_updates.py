...
from db2_flattener.schema import generate as generate_constants
...
def run_against(monkeypatch, profiles):
    ...
    monkeypatch.setattr(generate_constants, "create_json_profiles", lambda connection: profiles)
...
def test_object_config_only_change_is_saved(yaml_path, monkeypatch):
    ...
    assert after.object_config["human_donors"]["references"] == {"sex": "controlled_terms"}
    assert after.object_config_sha256 != before.object_config_sha256
    assert after.field_types_sha256 == before.field_types_sha256
